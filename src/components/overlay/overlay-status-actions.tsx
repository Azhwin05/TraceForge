...
} from "@/app/(app)/overlay-reports/actions"
import type { OverlayReportStatus, UserRole } from "@/types/database"
...
interface Props {
  reportId: string
  reportStatus: OverlayReportStatus
  userRole: UserRole
  generatedPdfPath: string | null
}

export function OverlayStatusActions({
  reportId,
  reportStatus,
  userRole,
  generatedPdfPath,
  ...
  async function handleGeneratePdf() {
    setLoading("generate")
    try {
      const resp = await fetch(`/api/overlay-reports/${reportId}/generate`, { method: "POST" })
  ...
  async function handleApprove() {
    ...
    setLoading("approve")
    const result = await approveOverlayReport(reportId, approvedByName)
    setLoading(null)
  ...
  async function handleReject() {
    ...
    setLoading("reject")
    const result = await rejectOverlayReport(reportId, rejectionReason)
    setLoading(null)
    if (result.error) { toast.error(result.error); return }
  ...
  async function handleSubmit() {
    setLoading("submit")
    const result = await markOverlaySubmitted(reportId)
    setLoading(null)
    if (result.error) { toast.error(result.error); return }
  ...
  return (
    <div className="space-y-4">
            ...
            <Button
              ...
              onClick={async () => {
                if (downloadUrl) {
                  ...
                  const resp = await fetch(`/api/overlay-reports/${reportId}/generate-url`)
                  const data = await resp.json()
      ...
      {/* Approve */}
      {isAdminOrQa && reportStatus === "draft" && (
        <div className="rounded-lg border border-border p-4 space-y-3">
          <h3 className="text-sm font-semibold">Approve Report</h3>
          <div>
            <Label htmlFor="overlayApprovedBy">Approved By (name)</Label>
            <Input
              id="overlayApprovedBy"
      ...
      {/* Reject */}
      {isAdminOrQa && (reportStatus === "draft" || reportStatus === "approved") && (
        <>
          {!showRejectForm ? (
            <Button
              size="sm"
            ...
            <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4 space-y-3">
              ...
              <div>
                <Label htmlFor="overlayRejReason">Reason <span className="text-destructive">*</span></Label>
                <textarea
                  id="overlayRejReason"
      ...
      {/* Mark Submitted */}
      {isAdminOrQa && reportStatus === "approved" && (
